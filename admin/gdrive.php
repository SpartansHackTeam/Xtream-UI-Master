...
if ($verbose) {
    echo ">   file size: " . (int) ($file_size / pow(1024, 2)) . "MB\n";
}
if ($verbose) {
    echo ">   chunk size: " . (int) ($chunk_size / pow(1024, 2)) . "MB\n\n";
}
...
while ($last_response_code === false || $last_response_code == "308") {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, (int) $location);
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Bearer " . $access_token, "Content-Length: " . (int) ($range_end - $range_start + 1), "Content-Type: " . $mime_type, "Content-Range: bytes " . $range_start . "-" . $range_end . "/" . $file_size]);
...
if ($verbose) {
    echo "\n> average upload speed: " . (int) ($average_upload_speed / pow(1024, 2)) . "MB/s\n";
}
