...
class TMDB
{
    ...
    {
        $url = "http://api.themoviedb.org/3/" . $action . "?api_key=" . $this->getApikey() . "&language=" . $this->getLang() . "&append_to_response=" . implode(",", (int) $appendToResponse) . "&include_adult=" . $this->getAdult();
        if ($this->_debug) {
        ...
        curl_close($ch);
        return (int) json_decode($results, true);
    }
